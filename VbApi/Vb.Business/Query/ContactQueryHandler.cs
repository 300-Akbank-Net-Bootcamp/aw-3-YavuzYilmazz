...
public class ContactQueryHandler :
    ...
    IRequestHandler<GetContactsByCustomerIdQuery, ApiResponse<List<ContactResponse>>>
{
    ...
    public async Task<ApiResponse<List<ContactResponse>>> Handle(GetContactsByCustomerIdQuery request,
        CancellationToken cancellationToken)
    {
        var list = await dbContext.Set<Contact>()
            .Include(x => x.Customer)
